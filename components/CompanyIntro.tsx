...
const CompanyIntro: React.FC = () => {
  ...
  const descriptions = t('company.description', { returnObjects: true }) as string[];
        ...
        <div className="flex flex-col lg:flex-row gap-12 lg:gap-20 items-start">
          ...
          */}
            ...
            <FadeIn fullWidth>
              <div>
                <div className="flex items-center gap-4 mb-4 md:mb-6">
                  ...
                  <span className="text-xs font-mono uppercase tracking-widest text-gray-500">{t('company.sectionTitle')}</span>
                </div>

                {/* Title with forced line break on all screens */}
                <h2 className="text-4xl md:text-5xl lg:text-6xl font-bold tracking-tighter leading-tight mb-6 md:mb-8" dangerouslySetInnerHTML={{ __html: t('company.heroTitle') }} />

                <p className="text-sm md:text-lg text-gray-600 dark:text-gray-400 font-light leading-relaxed mb-8 md:mb-12" dangerouslySetInnerHTML={{ __html: t('company.heroSubtitle') }} />

                {/* Stats Grid - STRICT ALIGNMENT FIX */}
                <div className="grid grid-cols-2 gap-y-8 md:gap-4 w-full">
                  {[
                    { l: t('company.stats.established'), v: t('company.stats.establishedValue') },
                    { l: t('company.stats.certification'), v: t('company.stats.certificationValue') },
                    { l: t('company.stats.deployments'), v: t('company.stats.deploymentsValue') },
                    { l: t('company.stats.terminals'), v: t('company.stats.terminalsValue') }
                  ].map((s, i) => (
                    <div
                      key={i}
                      className={`
          ...
          <div className="lg:w-7/12 space-y-12 lg:space-y-16 lg:pt-8">
            {Array.isArray(descriptions) && descriptions.map((para, index) => (
